...
public class StdCoord implements Coord {
	...
	public StdCoord(int row, int col) {
		Contract.checkCondition(isValidCol(col) || isValidRow(row), "Coordonnée invalide");
		this.col = col;
		this.row = row;
	}
	...
	public boolean equals(Object coord) {
		return coord != null && coord.getClass() == getClass() && ((Coord) coord).getCol() == col
				&& ((Coord) coord).getRow() == row;
	}

	@Override
	public boolean isAlignedWith(Coord coord, Direction dir) {
		Contract.checkCondition(coord != null, "coordonnée est nul");
		Contract.checkCondition(dir != null, "direction est nul");
		// False
		int delta = 1;
